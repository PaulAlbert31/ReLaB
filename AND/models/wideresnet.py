...
class WRN(nn.Module):
    ...
    def __init__(self, width, num_classes, transform_fn=None, long=False):
        super().__init__()
        if long:
        ...
        if True:
            self.unit4 = nn.Sequential(*[BatchNorm2d(filters[3]), relu(), nn.AdaptiveAvgPool2d((2,2))]) 
            
            self.output = nn.Linear(filters[3]*4, num_classes)
        else:
...
def WRN28_2(low_dim=10, **kwargs):
    return WRN(width=2, num_classes=low_dim, **kwargs)

def WRN28_10(num_class=10):
    return WRN(width=2, num_classes=num_class)
