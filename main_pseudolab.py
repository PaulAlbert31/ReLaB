...
from datasets.cifar_pseudolab import get_dataset
...
def data_config(args, transform_train, transform_test, dst_folder):
    ...
    ####################################### Train ##########################################################
    trainset, clean_labels, noisy_labels, train_noisy_indexes, train_clean_indexes, valset = get_dataset(args, transform_train, transform_test, dst_folder, dataset=args.dataset)
...
def record_params(args):
    dst_folder = args.out + '/' + args.dataset_type + '-lr-{}-ratio-{}-alpha-{}-beta-{}-{}'.format(args.lr,
                                                                                                   np.load(args.labeled_samples)['arr_0'].shape[0],
                                                                                                   args.alpha,
                                                                                                   args.beta,
                                                                                                   args.network)
    ...
    rd.write('labeled_samples:%f' % np.load(args.labeled_samples)['arr_0'].shape[0] + '\n')
    rd.close()
...
def main(args, dst_folder):
    ...
    if args.network == "WRN28_2":
        print("Loading WRN28_2...")
        from nets.wideresnet import WRN28_2
        model = WRN28_2(num_classes = args.num_classes).to(device)
    ...
    exp_path = os.path.join('./', 'noise_models_{0}'.format(args.experiment_name), str(np.load(args.labeled_samples)['arr_0'].shape[0]))
    res_path = os.path.join('./', 'metrics_{0}'.format(args.experiment_name), str(np.load(args.labeled_samples)['arr_0'].shape[0]))
    ...
    if args.load is not None:
        dict = torch.load(args.load)
        if 'state_dict' in list(dict.keys()):
            dict = dict['state_dict']
        elif 'network' in list(dict.keys()):
            dict = dict['network']
        elif 'net' in list(dict.keys()):
            dict = dict['net']
            
        if 'module' in list(dict.keys())[0]:
        ...
        if args.network == 'WRN28_2':
            ...
            del dict['output.bias']
        elif args.network == 'MT_Net':
            del dict['fc1.weight_g']
            del dict['fc1.weight_v']
            del dict['fc1.bias']
        else:
            del dict['linear.weight']
            del dict['linear.bias']

        model.load_state_dict(dict, strict=False)
        print('Loaded unsupervised weights')
        
    if args.freeze:
        lim = len(list(model.children())) - 2 #last block + linear
    ...
    for epoch in range(1, args.epoch + 1):
        ...
        scheduler.step()
        # train for one epoch
        print(args.experiment_name, np.load(args.labeled_samples)['arr_0'].shape[0])
        ...
        if epoch == 1:
            ...
            snapBest = 'best_epoch_%d_valLoss_%.5f_valAcc_%.5f_noise_%d_bestAccVal_%.5f' % (
                epoch, loss_per_epoch[-1], acc_val_per_epoch_i[-1], np.load(args.labeled_samples)['arr_0'].shape[0], best_acc_val)
        ...
        if (epoch >= 150) and (epoch%2 == 0) and (args.save_checkpoint == "True"):
            print("Saving model ...")
            out_path = './checkpoints/ENS_{0}_{1}'.format(args.experiment_name, np.load(args.labeled_samples)['arr_0'].shape[0])
        ...
        if cond and save:
            print("Saving models...")
            path = './checkpoints/{0}_{1}_{2}_{3}_{4}_S{5}.hdf5'.format(name, epoch, args.dataset, np.load(args.labeled_samples)['arr_0'].shape[0], args.network, args.seed)
        ...
        # Save losses:
        np.save(res_path + '/' + str(np.load(args.labeled_samples)['arr_0'].shape[0]) + '_LOSS_epoch_train.npy', np.asarray(loss_train_epoch))
        np.save(res_path + '/' + str(np.load(args.labeled_samples)['arr_0'].shape[0]) + '_LOSS_epoch_val.npy', np.asarray(loss_val_epoch))

        # save accuracies:
        np.save(res_path + '/' + str(np.load(args.labeled_samples)['arr_0'].shape[0]) + '_accuracy_per_epoch_train.npy',
                np.asarray(acc_train_per_epoch))
        np.save(res_path + '/' + str(np.load(args.labeled_samples)['arr_0'].shape[0]) + '_accuracy_per_epoch_val.npy', np.asarray(acc_val_per_epoch))

        # save the new labels
        new_labels.append(train_loader.dataset.targets)
        np.save(res_path + '/' + str(np.load(args.labeled_samples)['arr_0'].shape[0]) + '_new_labels.npy',
    ...
    if args.swa == 'True':
        ...
        snapLast = 'last_epoch_%d_valLoss_%.5f_valAcc_%.5f_noise_%d_bestValLoss_%.5f_swaAcc_%.5f' % (
            epoch, loss_per_epoch[-1], acc_val_per_epoch_i[-1], np.load(args.labeled_samples)['arr_0'].shape[0], best_acc_val, acc_val_swa[0])
        torch.save(model.state_dict(), os.path.join(exp_path, snapLast + '.pth'))
        torch.save(optimizer.state_dict(), os.path.join(exp_path, 'opt_' + snapLast + '.pth'))
